    ...
    def book(self, start: int, end: int) -> bool:
        ...
        start_frequency = (
            ...
            if start_index <= 0
                ...
                self.calendar[start_index - 1][1] + 1
                if self.calendar[start_index - 1][1] > 0
                else -self.calendar[start_index - 1][1]
            )
        )
        end_frequency = (
            -1
            if end_index >= len(self.calendar)
            else (
                self.calendar[end_index][1] - 1
                if self.calendar[end_index][1] < 0
                else -self.calendar[end_index][1]
            )
        )
